...
(defvar ac-auto-start-chars nil)
...
(defvar ac-complete-mode-map
  (let ((map (make-sparse-keymap)))
    ...
    (define-key map "(" 'ac-complete-and-self-insert)
    (define-key map ")" 'ac-complete-and-self-insert)
    (define-key map "[" 'ac-complete-and-self-insert)
    (define-key map "]" 'ac-complete-and-self-insert)
    (define-key map " " 'ac-complete-and-self-insert)
...
(defun ac-complete-and-self-insert ()
  "Try completion and self-insert the key."
  (interactive)
  (ac-complete)
  ...
  (progn                                ; ignore-errors
    (if (and (or ac-auto-start
                 ...
                 (and (eq this-command 'self-insert-command)
                      (member (this-command-keys) ac-auto-start-chars))
                 )
             (not isearch-mode)
