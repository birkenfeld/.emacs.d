...
See the varaible `ido-cr+-replace-completely' for more information." (when (not (featurep (quote ido-completing-read+))) (require (quote ido-completing-read+))) (if (or (ido-cr+-active) (not ido-cr+-replace-completely)) (apply orig-fun args) (apply (function ido-completing-read+) args)))

(define-advice call-interactively (:around (orig-fun command &rest args) ido-cr+-record-current-command) "\
Let-bind the command being interactively called.

See `ido-cr+-current-command' for more information." (let ((ido-cr+-current-command command)) (apply orig-fun command args)))
